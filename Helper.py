...
@jit #Check overlaps between particles and returns (number of overlaps + number of particles) (radius of a particle is set to 1)
def check_overlap(dist):
        dist_sq = (dist[:,:,0]*dist[:,:,0]+dist[:,:,1]*dist[:,:,1]+dist[:,:,2]*dist[:,:,2]) 
        exitcode = jnp.where(dist_sq < 4., 1., 0.)
        return exitcode #return 0 if 
