...
Nsteps = 100          # (int) Number of simulation timestep to perform
N=1000                 # (int) Number of particles in the simulation
writing_period = 100  # (int) Period for writing to file
L=30.                 # (float) Box size 
dt=0.001              # (float) timestep

init_positions_seed= 864645 #seed for creating initial configuration of non-overlapping particles (TO CHECK!)
positions = Helper.CreateRandomConfiguration(L, N, init_positions_seed) # (N,3) array of particle positions
...
shear_rate_0 = 0.01 # (float) Shear rate amplitude
shear_freq = 0.     # (float) Shear frequency
U=10.*shear_rate_0  # (float) Strength of colloidal bonds
...
main.main(
     ...
     U,       #strength of bonds
     1.,      #potential cutoff (1.5*colloid_diameter for attractive, 1*colloid_diameter for purely repulsive)
     'LJhe',  #type of potential (not needed for now, as there is only one type of potential)
     positions,
