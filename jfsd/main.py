...
from jfsd import applied_forces, ewald_tables, mobility, resistance, shear, thermal, utils
...
def main(
    ...
    def update_positions(
        shear_rate,  # shear rate at current time
        ...
        displacements_vector_matrix = (space.map_product(displacement))(
            positions, positions
        )
    ...
    eta, gaussP = utils.Check_max_shear(gridh, xisq, Nx, Ny, Nz, max_strain, error)
    ...
    m_self = m_self.at[0].set(
        (1 + 4 * pi12 * axi * math.erfc(2.0 * axi) - math.exp(-4.0 * axi2))
        / (4.0 * pi12 * axi * a)
    )
    ...
    ResTable_vals = utils.ResTable_dist
    ...
    lub_neighbor_fn, prec_lub_neighbor_fn, ff_neighbor_fn, pot_neighbor_fn = (
        utils.initialize_neighborlist(U_cutoff, Lx, Ly, Lz, displacement, ewald_cut)
    )
    ...
    for step in range(Nsteps):
        ...
        R_fu_prec_lower_triang, diagonal_elements_for_brownian = (
            resistance.RFU_Precondition(
                ichol_relaxer,
        ...
        saddle_b = applied_forces.sumAppliedForces(
            N,
            AppliedForce,
            AppliedTorques,
            saddle_b,
        ...
        if T > 0:
            ...
            if N > 1:  # no lubrication for isolated particles
                ...
                while (stepnormnf > 0.0001) and (n_iter_Lanczos_nf < 150):
                    ...
                    buffer, stepnormnf, diag_nf = (
                        thermal.compute_nearfield_brownianforce(
                            N,
                            T,
                            dt,
                            random_array_nf,
            ...
            if (not math.isfinite(stepnormff)) or (
                (n_iter_Lanczos_ff > 150) and (stepnormff > 0.0001)
            ):
                raise ValueError(
            ...
            if (not math.isfinite(stepnormnf)) or (
                (n_iter_Lanczos_nf > 150) and (stepnormnf > 0.0001)
            ):
                raise ValueError(
        ...
        if shear_rate_0 != 0:
            ...
            displacement, shift = space.periodic_general(
                jnp.array(
                    [[Lx, Ly * xy, Lz * 0.0], [0.0, Ly, Lz * 0.0], [0.0, 0.0, Lz]]
                ),
                fractional_coordinates=False,
            )
            gridk = shear.compute_sheared_grid(
        ...
        if (step % writing_period) == 0:
            ...
            overlaps, overlaps2 = utils.check_overlap(displacements_vector_matrix)
    ...
    print(
        ...
        Nsteps,
        " steps is ",
        total_time_end - total_time_start - compilation_time,
        "or ",
        Nsteps / (total_time_end - total_time_start - compilation_time),
        " steps per second",
    )
