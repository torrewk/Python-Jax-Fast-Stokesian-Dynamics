...
def wrap_SD(
    ...
    brownian_drift_history = np.zeros((int(Nsteps/writing_period), N, 6), float)
    ...
    displacements_vector_matrix = (space.map_product(displacement))(positions, positions)
    np.save("displacement_matrix_bioli.npy", displacements_vector_matrix)
    ...
    for step in tqdm(range(Nsteps),mininterval=0.5): #use tqdm to have progress bar and TPS 
        ...
        if(T > 0):
            ...
            if((stresslet_flag > 0) and ((step % writing_period) == 0)):
                ...
                brownian_drift_history[int(step/writing_period), :, :] = np.reshape(brownian_drift, (2500, 6))
                np.save(output/"brownian_drift.npy", brownian_drift_history)
