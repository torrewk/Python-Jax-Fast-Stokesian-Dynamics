...
ResTable_dist = jnp.load(files(__package__) / "tables" / "ResTableDist.npy")
ResTable_vals = jnp.load(files(__package__) / "tables" / "ResTableVals.npy")
...
def Check_max_shear(gridh, xisq, Nx, Ny, Nz, max_strain, error):
    ...
    eta = (
        (2.0 * w / gaussm) * (2.0 * w / gaussm) * (xisq)
...
def Precompute_grid_distancing(
    gauss_P, gridh, tilt_factor, positions, N, Nx, Ny, Nz, Lx, Ly, Lz
):
                ...
                grid[i, j, k, :] = (
                    ...
                    * (
                        (
                            ...
                            - center_offsets[:, 0]
                            + tilt_factor
                            * (j - int(gauss_P / 2) - center_offsets[:, 1])
                        )
                        ** 2
...
def initialize_neighborlist(U_cut, Lx, Ly, Lz, displacement, ewald_cut):
    ...
    lub_neighbor_fn = partition.neighbor_list(
        displacement,  # Displacement metric
        # Box size
        jnp.array([Lx, Ly, Lz]),
        # r_cutoff=0.,  # Spatial cutoff for 2 particles to be neighbor
        r_cutoff=4.0,  # Spatial cutoff for 2 particles to be neighbor
    ...
    prec_lub_neighbor_fn = partition.neighbor_list(
        displacement,  # Displacement metric
        # Box size
        jnp.array([Lx, Ly, Lz]),
    ...
    ff_neighbor_fn = partition.neighbor_list(
        displacement,  # Displacement metric
        # Box size
        jnp.array([Lx, Ly, Lz]),
    ...
    pot_neighbor_fn = partition.neighbor_list(
        displacement,  # Displacement metric
        # Box size
        jnp.array([Lx, Ly, Lz]),
...
def check_overlap(dist):
    ...
    exitcode = jnp.where(dist_sq < 3.996, 1.0, 0.0)
    exitcode2 = jnp.where(dist_sq < 3.996, dist_sq, 0.0)
    exitcode2 = jnp.where(exitcode2 > 0, exitcode2, 0.0)
...
def precompute(
    ...
    all_indices_x = jnp.repeat(
        ...
        jnp.repeat(jnp.repeat(jnp.arange(gaussP), gaussP), gaussP),
    ...
    all_indices_z = jnp.repeat(
        ...
        jnp.resize(
            jnp.resize(jnp.arange(gaussP), gaussP * gaussP), gaussP * gaussP * gaussP
        ),
        gaussP * gaussP * gaussP * N,
    )
    ...
    f1 = (
        tewaldC1m.at[:, 0].get()
        + (tewaldC1p.at[:, 0].get() - tewaldC1m.at[:, 0].get()) * fac_ff
    )
    f2 = (
        tewaldC1m.at[:, 1].get()
        + (tewaldC1p.at[:, 1].get() - tewaldC1m.at[:, 1].get()) * fac_ff
    )

    g1 = (
        tewaldC1m.at[:, 2].get()
        + (tewaldC1p.at[:, 2].get() - tewaldC1m.at[:, 2].get()) * fac_ff
    )
    g2 = (
        tewaldC1m.at[:, 3].get()
        + (tewaldC1p.at[:, 3].get() - tewaldC1m.at[:, 3].get()) * fac_ff
    )

    h1 = (
        tewaldC2m.at[:, 0].get()
        + (tewaldC2p.at[:, 0].get() - tewaldC2m.at[:, 0].get()) * fac_ff
    )
    h2 = (
        tewaldC2m.at[:, 1].get()
        + (tewaldC2p.at[:, 1].get() - tewaldC2m.at[:, 1].get()) * fac_ff
    )
    h3 = (
        tewaldC2m.at[:, 2].get()
        + (tewaldC2p.at[:, 2].get() - tewaldC2m.at[:, 2].get()) * fac_ff
    )
    ...
    XA11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 0] - ResTable_vals[22 * (ind) + 0])
            * fac_lub
        ),
        ResTable_vals[0],
    )
    ...
    XA12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 1] - ResTable_vals[22 * (ind) + 1])
            * fac_lub
        ),
        ResTable_vals[1],
    )
    ...
    YA11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 2] - ResTable_vals[22 * (ind) + 2])
            * fac_lub
        ),
        ResTable_vals[2],
    )
    ...
    YA12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 3] - ResTable_vals[22 * (ind) + 3])
            * fac_lub
        ),
        ResTable_vals[3],
    )
    ...
    YB11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 4] - ResTable_vals[22 * (ind) + 4])
            * fac_lub
        ),
        ResTable_vals[4],
    )
    ...
    YB12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 5] - ResTable_vals[22 * (ind) + 5])
            * fac_lub
        ),
        ResTable_vals[5],
    )
    ...
    XC11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 6] - ResTable_vals[22 * (ind) + 6])
            * fac_lub
        ),
        ResTable_vals[6],
    )
    ...
    XC12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 7] - ResTable_vals[22 * (ind) + 7])
            * fac_lub
        ),
        ResTable_vals[7],
    )
    ...
    YC11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 8] - ResTable_vals[22 * (ind) + 8])
            * fac_lub
        ),
        ResTable_vals[8],
    )
    ...
    YC12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 9] - ResTable_vals[22 * (ind) + 9])
            * fac_lub
        ),
        ResTable_vals[9],
    )
    ...
    XG11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 10] - ResTable_vals[22 * (ind) + 10])
            * fac_lub
        ),
        ResTable_vals[10],
    )
    ...
    XG12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 11] - ResTable_vals[22 * (ind) + 11])
            * fac_lub
        ),
        ResTable_vals[11],
    )
    ...
    YG11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 12] - ResTable_vals[22 * (ind) + 12])
            * fac_lub
        ),
        ResTable_vals[12],
    )
    ...
    YG12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 13] - ResTable_vals[22 * (ind) + 13])
            * fac_lub
        ),
        ResTable_vals[13],
    )
    ...
    YH11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 14] - ResTable_vals[22 * (ind) + 14])
            * fac_lub
        ),
        ResTable_vals[14],
    )
    ...
    YH12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 15] - ResTable_vals[22 * (ind) + 15])
            * fac_lub
        ),
        ResTable_vals[15],
    )
    ...
    XM11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 16] - ResTable_vals[22 * (ind) + 16])
            * fac_lub
        ),
        ResTable_vals[16],
    )
    ...
    XM12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 17] - ResTable_vals[22 * (ind) + 17])
            * fac_lub
        ),
        ResTable_vals[17],
    )
    ...
    YM11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 18] - ResTable_vals[22 * (ind) + 18])
            * fac_lub
        ),
        ResTable_vals[18],
    )
    ...
    YM12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 19] - ResTable_vals[22 * (ind) + 19])
            * fac_lub
        ),
        ResTable_vals[19],
    )
    ...
    ZM11 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 20] - ResTable_vals[22 * (ind) + 20])
            * fac_lub
        ),
        ResTable_vals[20],
    )
    ...
    ZM12 = jnp.where(
        ...
        (
            ...
            + (ResTable_vals[22 * (ind + 1) + 21] - ResTable_vals[22 * (ind) + 21])
            * fac_lub
        ),
        ResTable_vals[21],
    )
